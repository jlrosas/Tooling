...
class PaymentMethodsService extends __BaseService {
  static readonly getPaymentMethodsPath = '/rest/admin/v2/payment-methods';
  ...
  /**
   * Gets a collection of payment methods for the specified store.
   * @param params The `PaymentMethodsService.GetPaymentMethodsParams` containing the following parameters:
   *
   * - `storeId`: The unique numeric ID for identifying the store.
   *
   * - `languageId`: The ID of the language to be used to retrieve the payment method data.
   *
   * - `offset`: The position within the resulting dataset where the query begins returning item records. If the offset is "5", the records that returned begin with the sixth record that matches the query parameters. If the offset is "0", the records that are returned begin with the first record that matches the query parameters.
   *
   * - `limit`: The maximum number of records to return.
   *
   * - `name`: Limits search results to only include payment methods that match the specified name.
   *
   ...
   */
  getPaymentMethodsResponse(params: PaymentMethodsService.GetPaymentMethodsParams): __Observable<__StrictHttpResponse<any>> {
    ...
    let req = new HttpRequest<any>(
      'GET',
      this.rootUrl + `/rest/admin/v2/payment-methods`,
      __body,
      {
        headers: __headers,
        params: __params,
   ...
   * @return The request completed successfully.
   */
  getPaymentMethods(params: PaymentMethodsService.GetPaymentMethodsParams): __Observable<any> {
    return this.getPaymentMethodsResponse(params).pipe(
...
module PaymentMethodsService {
  ...
  /**
   * Parameters for getPaymentMethods
   */
  export interface GetPaymentMethodsParams {
    ...
    /**
     * The ID of the language to be used to retrieve the payment method data.
     */
    languageId?: number;
    ...
    /**
     * Limits search results to only include payment methods that match the specified name.
     */
    name?: string;
